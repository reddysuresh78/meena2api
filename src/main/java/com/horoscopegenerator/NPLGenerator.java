...
import com.horoscopegenerator.npl.pdf.NPLPdfCreatorPerDay2019;

import swisseph.DblObj;
import swisseph.SweDate;
import swisseph.SwissEph;

public class NPLGenerator
{
...
public NPLGenerator(String nameOfNative, double longitude, double latitude, String timeOfBirth, TimeZone timeZone, String placeName, boolean signsInEnglish)
    throws Exception
  {
...
//	  calendarOfBirth.add(Calendar.MINUTE, -30);
	...
	private BirthChart generateForSunRise() {
		double jut = 0.0D;
  ...
  public byte[] generate() throws Exception {
    ...
    iflag |= 256;
      ...
      planet.isRetrograde = (this.planets[2].isRetrograde = false);
      this.rasis[((int)Math.floor(this.moonLong / 30.0D))].planets.add(planet);
	  birthChart.setMoonLongitude(this.moonLong);
	  ...
	  String sunRiseTime = getDisplayDate(birthChart.getSunriseTime());
	  String newTime = this.timeOfBirth.substring(0, this.timeOfBirth.lastIndexOf(" ")+1) + sunRiseTime;

	  System.out.println("Sunrise time: " + te + "/" + sunRiseTime +  "/" + longitude + "/" + latitude);

	  NPLGenerator generatorForSunRise = new NPLGenerator("", this.longitude, this.latitude, newTime, this.timeZone, birthChart.getPlaceOfBirth(), true);
	  BirthChart sunriseChart = generatorForSunRise.generateForSunRise();
	  ...
	  String prevTime = getDisplayDateHHMM(sunriseChart.getSunriseTime());

	  sunriseChart.getSunriseTime().add(Calendar.SECOND, Math.round( balMinutesToAdd  * 60));

	  String newTime1 = getDisplayDateHHMM(sunriseChart.getSunriseTime());


	  Planet starLord = Star.valueOf(sunriseChart.getNakshatra()).getStarLord();
	  String npl = "";
	  for (PlanetInfo planetInfo : sunriseChart.getRaasiChakra().getPlanetInfo()) {
		  if (planetInfo.getPlanet() == starLord) {
			  npl = planetInfo.getHouse().getRaasi().getCamelCaseString();
			  break;
		  }
	  }
	  ...
	  prevTime = getDisplayDateHHMM(sunriseChart.getSunriseTime());
	  ...
	  for (int i = curOrdinal + 1; i < 27 ; i++  ){
		  	...
		  	newTime1 = getDisplayDateHHMM(sunriseChart.getSunriseTime());
	  ...
	  for (int i = 0; i <= curOrdinal  ;  i++  ){
  	  ...
  	  return new NPLPdfCreatorPerDay2019().generateLocalPdf(sunriseChart, filePath, imageData);
    }
  ...
  private void calculatePanchanga() {  
	    ...
	    String dob = getDDMMYYYY(this.displayCalendarOfBirthInLocalTime) + 
	  	      "  " + (this.displayCalendarOfBirthInLocalTime.get(11) < 10 ? "0" : "") + this.displayCalendarOfBirthInLocalTime.get(11) + ":" + (this.displayCalendarOfBirthInLocalTime.get(12) < 10 ? "0" : "") 
	  	      		+ this.displayCalendarOfBirthInLocalTime.get(12) + ":" + (this.displayCalendarOfBirthInLocalTime.get(13) < 10 ? "0" : "") + this.displayCalendarOfBirthInLocalTime.get(13) + " AM " +
