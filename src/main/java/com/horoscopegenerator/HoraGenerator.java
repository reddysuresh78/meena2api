...
/*
* This is the main class used from activity.
*
* */
...
public class HoraGenerator {
    ...
    public HoraGenerator(String nameOfNative, double longitude, double latitude, String timeOfBirth, int offset, TimeZone timeZone, String placeName, boolean signsInEnglish)
            throws Exception {
...
//        dfmt.setTimeZone(timeZone);
        ...
        System.out.println("Final date and time actualtime " + cal.getTime());
        System.out.println("Current time on Wake Island --> " + df.format(Calendar.getInstance(timeZone).getTime()));


        System.out.println("Final date and time actualtime " + cal.getTimeZone());
        cal.add(Calendar.MILLISECOND, -offset);

        //For war time, Add 1 hour. For dates war time, add 23 minutes: Only for India
        ...
        this.calendarOfBirth.setTime(cal.getTime());
//        this.calendarOfBirth.setTimeZone(TimeZone.getTimeZone("GMT"));

        System.out.println("Final date and time Before " + calendarOfBirth.getTime());

         //Make it GMT
        //calendarOfBirth.add(Calendar.HOUR_OF_DAY, -5);
        //calendarOfBirth.add(Calendar.MINUTE, -30);

        this.jday = this.calendarOfBirth.get(Calendar.DAY_OF_MONTH);
        this.jmon = (this.calendarOfBirth.get(Calendar.MONTH) + 1);
        this.jyear = this.calendarOfBirth.get(Calendar.YEAR);
        this.thour = (this.calendarOfBirth.get(Calendar.HOUR_OF_DAY) + this.calendarOfBirth.get(Calendar.MINUTE) / 60.0D + this.calendarOfBirth.get(Calendar.SECOND) / 3600.0D);

        System.out.println("Final date and time After " + calendarOfBirth.getTime());
        ...
        for (i = 0; i < this.rasis.length; i++) {
            this.rasis[i] = new CalculatedRasi();
            this.rasis[i].index = i;
            this.rasis[i].name = HoraStrings.RASHIS[i];

            this.navamsaRasis[i] = new CalculatedRasi();
        ...
        for (i = 0; i < HoraStrings.NAKSHATRA_LORDS.length; i++) {
            ...
            calendar.add(Calendar.YEAR , this.DASA_LENGTHS[i]);
...
//        this.sw.swe_set_topo(this.top_long, this.top_lat, top_elev);
        ...
        System.out.println("Sun: " + getDM(this.sunLong));

        //Calculate sunrise time as well.
        DblObj returnedTime = new DblObj();
        //Determine sun rise time
        sw.swe_rise_trans(te, 0, null, 4, 1, new double[]{this.top_long, this.top_lat, top_elev}, 0, 0, returnedTime, serr);
        ...
        System.out.println("Moon long " + this.moonLong);
        ...
        System.out.println("Moon: " + getDM(this.moonLong));
        ...
        for (int ipl = 2; ipl < this.PLANETS.length - 1; ipl++) {
            ...
            System.out.println(this.sw.swe_get_planet_name(this.PLANETS[ipl]) + ": " + getDM(x[0]) + (planet.isRetrograde ? " R" : ""));
        }
        ...
        this.rasis[((int) Math.floor(x[0] / 30.0D))].planets.add(planet);
        System.out.println("Rahu: " + getDM(x[0]));
        ...
        System.out.println("Ketu: " + getDM(ketuLong));
        ...
        System.out.println("Ascendant: " + getDM(ascmc[0]));
...
//            return new PdfGenerator().generatePdf(birthChart, "test.pdf");
    ...
    private void calculateNavamsa() {
        ...
        for (int i = 0; i < this.rasis.length; i++) {
            CalculatedRasi rasi = this.rasis[i];
    ...
    private void calculateDasas() throws Exception {
        ...
        tmp181_179[tmp181_180] = (tmp181_179[tmp181_180] + getDDMMYY(dasaTime));
        ...
        for (i = 0; i < nakshatraLord; i++) {
            ...
            tmp490_488[tmp490_489] = (tmp490_488[tmp490_489] + getDDMMYY(dasaTime));
            calculateAntarDasas(i, tmp332_331, dasaIndex);
        }

        for (i = 0; i <= dasaIndex; i++) {
            for (j = 0; j < dasaDetails[i].length; j++) {
//                System.out.println(dasaDetails[i][j]);
            }
        }
    ...
    private void calculatePanchanga() {
        ...
        String dob = getDDMMYYYY(this.displayCalendarOfBirthInLocalTime) +
                "  " + (this.displayCalendarOfBirthInLocalTime.get( Calendar.HOUR_OF_DAY) < 10 ? "0" : "") +
                        this.displayCalendarOfBirthInLocalTime.get( Calendar.HOUR_OF_DAY) + ":" +
                        (this.displayCalendarOfBirthInLocalTime.get(Calendar.MINUTE) < 10 ? "0" : "")
                      + this.displayCalendarOfBirthInLocalTime.get(Calendar.MINUTE) + ":" +
                        (this.displayCalendarOfBirthInLocalTime.get(Calendar.SECOND) < 10 ? "0" : "") +
                         this.displayCalendarOfBirthInLocalTime.get(Calendar.SECOND) +
        ...
        birthChart.setPlaceOfBirth(this.place);
        System.out.println("Placefbirth " + this.place);
        if (!birthChart.getPlaceOfBirth().contains("Long")) {
            birthChart.setPlaceOfBirth(this.place + " (Long: " + getDM(this.top_long) + " Lat: " + getDM(this.top_lat) + ")");
        }
        birthChart.setDay(weekDays[this.displayCalendarOfBirthInLocalTime.get(Calendar.DAY_OF_WEEK)]);
