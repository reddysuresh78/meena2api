...
public class NPLGeneratorPerDay2019 {

    private static final int SID_METHOD = SweConst.SE_SIDM_LAHIRI;

    public NPLGeneratorPerDay2019() {}

    //If this is called from activity
    public byte[] generate(NativeDetails nativeDetails ) throws Exception {
        return generate(nativeDetails, true, false);
    }

    public BirthChart generateForSunrise(NativeDetails nativeDetails, boolean signsInEnglish, boolean isLocal) throws Exception {
        ...
        //Determine sun rise time
        sw.swe_rise_trans( sd.getJulDay(), 0, null, 4 , 1 + sunriseFlags, new double[]{ longitude, latitude, 0}, 1013.25, 20, returnedTime, serr);
        ...
        System.out.println("SECOND: Sunrise time: " + sd.getJulDay() + "/" + sunRiseTime +  "/" + longitude + "/" + latitude);
...
//        calcJSAndGuna(birthChart);
    ...
    public byte[] generate(NativeDetails nativeDetails, boolean signsInEnglish, boolean isLocal) throws Exception {
...
//        System.out.println("Sunrise time: " + sd.getJulDay() + "/" + sunRiseTime +  "/" + longitude + "/" + latitude);
        ...
        String sunRiseTime = Utils.getDisplayDate(birthChart.getSunriseTime());
        System.out.println("FIRST: Sunrise time: " + sd.getJulDay() + "/" + sunRiseTime +  "/" + longitude + "/" + latitude);
...
//        birthChart.setSunriseTime(toDate(returnedTime.val, offsetHrs));
        ...
        String newTime = nativeDetails.getDateOfBirth() + " " + sunRiseTime;

        NPLGeneratorPerDay2019 generatorForSunRise = new NPLGeneratorPerDay2019();
...
//        "", longitude, latitude, newTime, TimeZone.getTimeZone(  nativeDetails.getTimeZoneId()), birthChart.getPlaceOfBirth(), true);
        BirthChart sunriseChart = generatorForSunRise.generateForSunrise(nativeDetails,true, true );
        sunriseChart.setSunriseTime(sunriseChart.getSunriseTime());
        String balNakshatra = sunriseChart.getBalanceNakshtra();

        nativeDetails.setTimeOfBirth(newTime);
        ...
        String prevTime = Utils.getDisplayDateHHMMSS(sunriseChart.getSunriseTime());


        System.out.println("SUNRISE TIME:" + Utils.getDisplayDateHHMMSS(sunriseChart.getSunriseTime()));

        sunriseChart.getSunriseTime().add(Calendar.SECOND, Math.round( balMinutesToAdd  * 60));

        String newTime1 = Utils.getDisplayDateHHMMSS(sunriseChart.getSunriseTime());
        ...
        prevTime = Utils.getDisplayDateHHMMSS(sunriseChart.getSunriseTime());
        ...
        for (int i = curOrdinal + 1; i < 27 ; i++  ){
            ...
            newTime1 = Utils.getDisplayDateHHMMSS(sunriseChart.getSunriseTime());
        ...
        return new NPLPdfCreatorPerDay2019().generateLocalPdf(sunriseChart, nativeDetails.getFilePath(), nativeDetails.getImageData());
...
//        calcJSAndGuna(birthChart);
    ...
    public static void calcJSAndGuna(BirthChart birthChart) throws Exception {
        ...
        for (Planet planet : Planet.values()) {
            ...
            Planet[] jsPlanets = getJeevaSareeraPlanets(birthChart, planet, currentPlanet, false);
